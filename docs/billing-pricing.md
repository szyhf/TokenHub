...
A card contains `kind` (`tenant`, `tenant_team`, or `provider`), `target` (model name, `team_id:model_name`, or `provider_id:upstream_model`). A `tenant_team` card prices one team's usage of one model: resolution prefers the team card and falls back to the global tenant card, and publishing validates that the target team exists so a typo cannot silently fall back. `currency`, `source`, optional `effective_from`, `rates`, and optional `periods`. Rate keys are `input`, `cache_read`, `cache_write`, `cache_write_5m`, `cache_write_1h`, and `output`. Periods combine the schedule fields above with a `rates` override object. The latest applicable effective time wins; immediate publications sharing a database timestamp use the card revision. Existing snapshots retain their original version. FX selection is frozen with each Provider attempt. Publishing a rate or FX version records an administrator audit event.
