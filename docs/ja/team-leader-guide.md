...
チームリーダーは、プロバイダーが自分のチームに属するモデルルートを管理します。**ルーティングポリシー**で外部モデルから自チャネルの取り込み済み上流モデルへのマッピングを作成し、優先度、重み、プロジェクトスコープを調整します。

- 学生のリクエストは、あなたが所有するプロジェクトを通じてルートを解決します。主チームがあなたのチームであるプロジェクトは自チャネルのルートとプラットフォームのルートを利用でき、他チームのプロジェクトはあなたのチャネルに決して到達しません。
- 同一の外部モデルにプラットフォームルートと自分のルートを併設できます。プランナーはあなたのチームのプロジェクトには自チャネルを、それ以外にはプラットフォームチャネルを優先度と重みで選択します。
- ルートの作成・編集では自分のチームが所有するプロバイダーのみを受け付け、ルート一覧もそれらのルートのみを表示します。グローバルなルーティング面（モデル単位のルーティング戦略、ルーティングポリシーオブジェクト、ポリシーバインディング）は引き続き管理者が管理します。
